...
describe("ContactImageClient", () => {
  ...
  it("Can fetch contact images metadata for an invalid id", async () => {
    let metadataUrl: string = '/contacts/12345/images';
    let res: ResponseData < ContactImageMetadata[] > = await contactClient.get < ContactImageMetadata[] > (metadataUrl);
    expect(res.success).toBeFalsy(res.message);
  });

  // it("Can fetch profile picture for a contact", async () => {
  //   let res: ResponseData < Image > = await contactClient.getContactImages(aValidContactId, 2);
  //   expect(res.success).toBeTruthy(res.message);
  // });

  // it("Can fetch front of the business card for a contact", async () => {
  //   let res: ResponseData < Image > = await contactClient.getContactImages(aValidContactId, 3);
  //   expect(res.success).toBeTruthy(res.message);
  // });

  // it("Can fetch back of the business card for a contact", async () => {
  //   let res: ResponseData < Image > = await contactClient.getContactImages(aValidContactId, 4);
  //   expect(res.success).toBeTruthy(res.message);
  // });

  // it("Can fetch all the images for an invalid image type", async () => {
  //   //returns all images by default or for an invalid image type
  //   let res: ResponseData < Image > = await contactClient.getContactImages(aValidContactId, 8);
  //   expect(res.success).toBeTruthy(res.message);
  // });

  it("Can upload a profile image for a contact", async () => {
    let res: ResponseData < Image > = await contactClient.updateContactImage(
      aValidContactId,
      'profilepicture',
      'image/jpeg',
      testImage
